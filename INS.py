...
# test 8
# RTK에서 나오는 속도값을 사용했으나 바이어스 발생
# 오히려 차분에서 사용한 값이 바이어스 발생이 없었음
# 그런데 규칙성은? RTK속도값을 사용할 때 더 일정했음
# 차분값은 조금 튈 때도 있음

# 주행한 경로를 저장하기로함
# 이 코드의 마지막은 차분값을 이용하여 구한것이다!!

# test9
# test8 결과값에서 RTK velocuty가 N,E가 바뀐거 같은데..??????
# 질문?
# lenar vel값이 NED 로 변환시 z가 부호가 바뀌면서 xy도 바뀌어서
# x가 e에 대응되고 y가 n에 대응되는건가? 왜..?? 대충 감은 잡히는데 이유는 모르겠음
# 그럼 z는..?

# test10
# yaw 초기값을 vn300값을 이용해본다
# 확실히 초기 직선구간은 vn300을 추종
# 이후에는 gps yaw값을 추종함
# RTk vel을 이용하였음
# 하지만, 아직 vel에서는 오차가 발생하는중

# Q수정해서 위치 정확도 2cm 높임
# dX0 값중 accel과 gyro값을 기하급수적으로 조절하니 오차가 엄청나게 줄어듬!!!!
# dX는 bias이다.
# bias는 차를 가만히 나뒀을때 당연히 0이 나와야하는데 이거를 모두 합해서 나눠주면됨(error)

# test11
# 차가 움직이지 않는동안 cnt의 따른 bias세팅
# bias를 바로 산출하고 이용하도록 함
# vn300_vs_kalman_MSE std :  3.9279723042502934
# vn300_vs_kalman_MSE mean :  3.4709930483706
# GPSyaw_vs_kalman_MSE std :  11.208951524674744
# GPSyaw_vs_kalman_MSE mean :  13.13087812304949
# pitch_MSE std :  3.431283141790867
# pitch_MSE mean :  5.32468338001501
# roll_MSE std :  3.480257348266775
# roll_MSE mean :  5.5755500644805975
# vn_MSE std :  0.1436460428178732
# vn_MSE mean :  0.15796063681074002
# ve_MSE std :  0.1601665254464529
# ve_MSE mean :  0.13365552073516293
# vd_MSE std :  0.05410533056926768
# vd_MSE mean :  0.05674141515395174

# INS데이터와 싱크가 맞지 않아 더이상 비교는 어려울듯
# 일단 연산량 줄이는것을 목표로 진행하기
# 이후 GPS yaw 값 산출이 맞는지 검증하기

# GPS는 0.2로 samplingtime을 강제하면
# 성능이 떨어짐 그냥 내버려두니 가끔 0.1나와도
# 성능이 훨씬 잘나옴(2도정도 차이남)

# sync1
# IMU출력 주기가 vn300이 20hz임에 따라
# dt를 0.05로 고정함

# 이후 IMU출력 주기와 GPS출력 주기 sync를 맞춰서 칼만 돌림
# 그전에는 오차가 4~5도였는데
# 1도까지 줄임
# 하지만 roll, pitc, velocity는 아직도 부정확함

# 무엇보다 fail safe가 안되는 상황임
# cnt가 꼬였음 해결해야함!!!
...
def vn100_Kalman_filter(current_lat, current_lon, current_alt, current_accel_x,
    ...
    while True:
        ...
        if GPS_CTC.value != 0:
            ...
            final_bias = []
            for i in range(len(cal_bias)):
                final_bias.append([
                    RMSE(cal_bias[i ,0], cal_bias[0, 0]),
                    RMSE(cal_bias[i, 1], cal_bias[0, 1]),
                    RMSE(cal_bias[i, 2], cal_bias[0, 2]),
                    RMSE(cal_bias[i, 3], 0),
                    RMSE(cal_bias[i, 4], 0),
                    RMSE(cal_bias[i, 5], 0),
                    RMSE(cal_bias[i, 6], cal_bias[0, 6]),
                    RMSE(cal_bias[i, 7], cal_bias[0, 7]),
                    RMSE(cal_bias[i, 8], cal_bias[0, 8]),
                    RMSE(cal_bias[i, 9], 0),
                    RMSE(cal_bias[i, 10], 0),
                    RMSE(cal_bias[i, 11], cal_bias[0, 11]),
                    RMSE(cal_bias[i, 12], 0),
                    RMSE(cal_bias[i, 13], 0),
                    RMSE(cal_bias[i, 14], 0)
                ])
            final_bias = np.array(final_bias)

            dX0 = [
                [np.mean(final_bias[:,0])],
                [np.mean(final_bias[:, 1])],
                [np.mean(final_bias[:, 2])],
                [np.mean(final_bias[:, 3])],
                [np.mean(final_bias[:, 4])],
                [np.mean(final_bias[:, 5])],
                [np.mean(final_bias[:, 6])],
                [np.mean(final_bias[:, 7])],
                [np.mean(final_bias[:, 8])],
                [np.mean(final_bias[:, 9])],
                [np.mean(final_bias[:, 10])],
                [np.mean(final_bias[:, 11])],
                [np.mean(final_bias[:, 12])],
                [np.mean(final_bias[:, 13])],
                [np.mean(final_bias[:, 14])]]
            ...
            X0 = np.array([current_lat.value, current_lon.value, current_alt.value,
                              GPS_fix_velocity_y.value, GPS_fix_velocity_x.value, GPS_fix_velocity_z.value,
                              0.7365554571, 1.463993669,
                              128.7742004])  # set initial state  RTK 와 IMU 를 이용했음
            ...
            while True:
                ...
                # print(GPS_CNT.value)
                # print(G_CNT)
                # print(G_CNT_emergency)
                # using cnt, take accurately sync
                while True:
                    GPS_Integration = 0
                ...
                #
                #         plt.xlim(Kalman_E - 1, Kalman_E + 1)
                #         plt.ylim(Kalman_N - 1, Kalman_N + 1)
                #
